...
@endif
  <li><a href="#">Đổi mật khẩu</a></li>
        ...
        <div class="profile-info">
            <h2>Lập hóa đơn cho bệnh nhân</h2>
                ...
                <tbody>
                    @foreach($appointments as $appointment)
                        @php
                            ...
                            $total = $prescription->prescriptionDetails->sum(function($detail) {
                               $total1 = $detail->quantity * $detail->medicines->price + $detail->quantity;
                                
                                return $total1 + $detail->prescription->appointment->service->price*1/2;
                            });
                        @endphp
                        <tr>
                            <td>{{ $appointment->patient->user->name }}</td>
                            <td>{{Carbon\Carbon::parse($appointment->time)->format('d/m/Y') }}</td>
                            ...
                            <td>
                                ...
                                {{ $loop->iteration }}. {{ $detail->medicines->name ?? 'Thuốc không xác định' }} :  {{$detail->quantity}} Viên <br>

                              
                            @endforeach
                            </td>
                            
                            <td>{{ $appointment->service->name }} :{{ number_format($appointment->service->price) }} đ</td>
                            <td>{{ number_format($total) }} đ</td>
                            <td>
                                <form action="{{ route('invoices.store') }}" method="POST">
                                    @csrf
                                    <input type="hidden" name="prescription_id" value="{{ $appointment->prescription->prescription_id }}">
                                    <input type="hidden" name="appointment_id" value="{{ $appointment->appointment_id }}">
                                    <input type="hidden" name="patient_id" value="{{ $appointment->patient_id }}">
                                    <input type="hidden" name="total_amount" value="{{ $total }}">
                                    <input type="submit" class="btn btn-sm btn-primary" value="Tạo hóa đơn">
                                </form>
                            </td>
                        </tr>
                    @endforeach
