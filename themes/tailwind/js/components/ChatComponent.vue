...
const sendMessage = () => {
    if (newMessage.value.trim() !== "") {
        axios
            .post(`/messages/${props.friend.user_id}`, {
...
const sendTypingEvent = () => {
    Echo.private(`chat.${props.friend.user_id}`).whisper("typing", {
        userID: props.currentUser.user_id,
    });
};

onMounted(() => {
    axios.get(`/messages/${props.friend.user_id}`).then((response) => {
        console.log(response.data);
        messages.value = response.data;
    });

    Echo.private(`chat.${props.currentUser.user_id}`)
        .listen("MessageSent", (response) => {
            messages.value.push(response.message);
        })
        .listenForWhisper("typing", (response) => {
            isFriendTyping.value = response.userID === props.friend.user_id;
